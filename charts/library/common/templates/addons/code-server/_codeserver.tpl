...
*/}}
{{- define "geek-cookbook.common.addon.codeserver" -}}
{{- if .Values.addons.codeserver.enabled -}}
  {{/* Append the code-server container to the additionalContainers */}}
  {{- $container := include "geek-cookbook.common.addon.codeserver.container" . | fromYaml -}}
  {{- if $container -}}
    {{- $_ := set .Values.additionalContainers "addon-codeserver" $container -}}
  {{- end -}}

  {{/* Include the deployKeySecret if not empty */}}
  {{- $secret := include "geek-cookbook.common.addon.codeserver.deployKeySecret" . -}}
  {{- if $secret -}}
    {{- $secret | nindent 0 -}}
  {{- end -}}

  {{/* Append the secret volume to the volumes */}}
  {{- $volume := include "geek-cookbook.common.addon.codeserver.deployKeyVolumeSpec" . | fromYaml -}}
  {{- if $volume -}}
  ...
  {{- if .Values.addons.codeserver.service.enabled -}}
    ...
    {{- include "geek-cookbook.common.classes.service" $ -}}
  ...
  {{- if .Values.addons.codeserver.ingress.enabled -}}
    ...
    {{- $svcName := printf "%v-codeserver" (include "geek-cookbook.common.names.fullname" .) -}}
