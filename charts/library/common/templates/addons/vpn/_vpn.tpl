...
*/}}
{{- define "geek-cookbook.common.addon.vpn" -}}
{{- if .Values.addons.vpn.enabled -}}
  {{- if eq "openvpn" .Values.addons.vpn.type -}}
    {{- include "geek-cookbook.common.addon.openvpn" . }}
  {{- end -}}

  {{- if eq "wireguard" .Values.addons.vpn.type -}}
    {{- include "geek-cookbook.common.addon.wireguard" . }}
  {{- end -}}

  {{- if eq "gluetun" .Values.addons.vpn.type -}}
    {{- include "geek-cookbook.common.addon.gluetun" . }}
  {{- end -}}

  {{/* Include the configmap if not empty */}}
  {{- $configmap := include "geek-cookbook.common.addon.vpn.configmap" . -}}
  {{- if $configmap -}}
    {{- $configmap | nindent 0 -}}
  {{- end -}}

  {{/* Include the secret if not empty */}}
  {{- $secret := include "geek-cookbook.common.addon.vpn.secret" . -}}
  {{- if $secret -}}
    {{- $secret | nindent 0 -}}
  {{- end -}}

  {{/* Append the vpn scripts volume to the volumes */}}
  {{- $scriptVolume := include "geek-cookbook.common.addon.vpn.scriptsVolumeSpec" . | fromYaml -}}
  {{- if $scriptVolume -}}
  ...
  {{- $configVolume := include "geek-cookbook.common.addon.vpn.configVolumeSpec" . | fromYaml }}
  {{ if $configVolume -}}
  ...
  {{- $networkpolicy := include "geek-cookbook.common.addon.vpn.networkpolicy" . -}}
  {{- if $networkpolicy -}}
